...
try:
    ...
    # --- Cancellation Processing Engine ---
    def process_cancellation_file(uploaded_file):
        ...
        if "FILE_NO" not in df.columns or "BRANCH" not in df.columns: return None
    ...
    if active_branches:
        ...
        if cxl_file:
            ...
            if df_cxl is not None and not df_cxl.empty:
                ...
                total_df = total_df.merge(df_cxl, left_on=["FILE_NO", "Branch"], right_on=["FILE_NO", "BRANCH"], how="left", indicator=True)
                ...
                for name in list(active_branches.keys()):
                    ...
                    if active_branches[name].empty: del active_branches[name]
